...
class Cuboid(Obstacle):
    ...
    def __init__(self, reference_point, orientation, dimensions, safety_factor=1, reactivity=1, repulsion_coeff=1):
    ...
    def get_basis_matrix(self, pos) -> np.array:
        ...
        no_face = np.all(face_agreement > 0, axis=1)
        if self.ref_pos.shape[0] == 2: # 2 dimensional
            # use circular coordinates to get tangential vector to surface
            e[no_face,0,0] = -n[no_face,1]
            e[no_face,0,1] = n[no_face,0]

        elif self.ref_pos.shape[0] == 3: # 3 dimensional
            # use spherical coordinates to get tangential vectors to surface
            cosTheta = n[no_face,2]
            theta = np.arccos(cosTheta)
            sinTheta = np.sin(theta)
            cosPhi = n[no_face,0] / sinTheta
            sinPhi = n[no_face,1] / sinTheta

            # Theta unit vector
            e[no_face,0,0] = cosTheta * cosPhi
            e[no_face,0,1] = cosTheta * sinPhi
            e[no_face,0,2] = -sinTheta

            # Phi unit vector
            e[no_face,1,0] = -sinPhi
            e[no_face,1,1] = cosPhi
    ...
    def plot_obstacle(self, ax, color='g', show=False) -> None:
        if self.ref_pos.shape[0] == 2:
            ...
            rectangle = Rectangle(anchor, self.dimensions[0], self.dimensions[1], 180 / np.pi * np.arctan2(self.orientation[0,1], self.orientation[0,0]), color=color)
